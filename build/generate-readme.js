...
const renderRuleTitle = filename =>
  fs.existsSync(
    path.resolve(path.join(__dirname, '..', 'docs', 'rules', `${filename}.md`))
  )
...
const renderTags = (tags, fixFrom) => `<details>
  <summary>Included in ${
    resolveTags({ tags, fixFrom }).length
  } configs</summary>
...
${rules
  .map(
    ({
      meta: {
        docs: { deprecated, removed, fixFrom, recommended },
        fixable,
      },
      [FILENAME]: filename,
    }) =>
      `| [${filename}](#frogeducationjquery-compat${filename.replace(
        '/',
        ''
      )}) | ${deprecated || removed || ''} | ${
        fixFrom && semver.satisfies(fixFrom, '>=1.0.0')
          ? fixFrom
          : '(must rewrite)'
      } | ${removed || ''} | ${fixable ? 'Yes' : ''} | ${
        recommended ? 'Yes' : ''
...
const rules = glob
  .sync(path.join(__dirname, rulesDir, '**', '*.js'))
  .map(filename =>
    path.relative(__dirname, filename).slice(rulesDir.length + 1)
  )
  .sort()
  .map(filename =>
    [
...
const result = rules
  .map(
    ({
      meta: {
        docs: { description, detail, tags, fixFrom },
      },
      [FILENAME]: filename,
    }) => {
...
const built = Object.entries({ SUPPORTED_RULES, SUPPORTED_RULES_TABLE }).reduce(
  (acc, [key, value]) => acc.replace(`%%%${key}%%%`, value),
  template
)

fs.writeFileSync(path.join(__dirname, '..', 'README.md'), built)
