  ...
  const resolveProperty = memberExpression => {
    ...
    if (resolved === UNRESOLVED) {
      if (memberExpression.property.type === 'Identifier') {
        ...
        resolved = `UNRESOLVED_${context
          .getSourceCode()
          .getText(memberExpression.property)}`
      }
    }
...
const applyJsonPatchToObject =
  ({ createParentObjectsIfMissing } = {}) =>
  object =>
  patch => {
    ...
    let deepestObject = pathBits
      .slice(0, -1)
      .reduce((parent, bit) => parent[bit] || {}, object)
...
const resolveIdentifierValue = (node, context, resolve) => {
  const result = (() => {
    ...
    const writeExprs = refsWithWrite.map(ref => ref.writeExpr).filter(truthy)
    const beforeIdentifierNode = writeExprs.filter(
      write => write.end < node.end
    )
    ...
    if (value && typeof value.value === 'object') {
      const assignmentExpressions = allReferences
        ...
        .filter(parent => parent.left.type === 'MemberExpression')
      ...
      assignmentExpressions.forEach(
        applyJsonPatchToObject({ createParentObjectsIfMissing: true })(
          value.value
        )
      )
