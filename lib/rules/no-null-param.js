  ...
  meta: {
    docs: {
      ...
      detail: {
        ...
        examples: {
          incorrect: ['$.param({ foo: null, bar: undefined, baz: 42 })'],
          correct: [
    ...
    messages: {
      'no-null-param':
        'Cannot encode {{value}} in jQuery.param() on path {{path}}',
      'no-null-ajax-data':
        'Cannot encode {{value}} in data key passed to $.ajax on path {{path}}',
    },
    type: 'problem',
  },
  create: context => ({
    CallExpression: node => {
      ...
      const relevantArgumentNode = isParam
        ? node.arguments[0]
