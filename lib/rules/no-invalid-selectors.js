...
const jQueryPseudoFunctions = ['eq', 'gt', 'has', 'lt']

const otherPseudoCorrections = ['hover']
...
module.exports = {
  meta: {
    docs: {
      ...
      detail: {
        body:
          '**Cause:** Sizzle may throw errors if an invalid selector is passed to a jQuery function.\n' +
          '\n' +
  ...
  create: detectJQueryCollections((context, { foundAPositiveMatch }) => {
    ...
    const { allowJQueryExtensions, customPseudoClasses } = optionsObject
    ...
    return {
      'CallExpression:exit': node => {
        ...
        let replacements = otherPseudoCorrections
          .concat(customPseudoClasses)
          .map(pseudoClass => [pseudoClass, 'empty'])
        if (allowJQueryExtensions) {
          replacements = replacements
            .concat(
              jQueryPseudoFunctions.map(pseudoFunc => [pseudoFunc, 'not'])
            )
            .concat(
              jQueryPseudoClasses.map(pseudoFunc => [pseudoFunc, 'empty'])
            )
        }
        let correctedSelector = replacements.reduce(
          (selector, [search, replace]) =>
            selector.replace(
              RegExp(`:${search}([^-a-zA-Z0-9]|$)`, 'g'),
              `:${replace}$1`
            ),
          selector
        )
