...
const BANNED_ATTRS = ['selected', 'checked', 'disabled']
  ...
  meta: {
    docs: {
      ...
      detail: {
        ...
        examples: {
          incorrect: BANNED_ATTRS.map(
            attr => `if ($foo.attr("${attr}")) { /* ... */ }`
          ),
          correct: BANNED_ATTRS.map(
            attr => `if ($foo.prop("${attr}")) { /* ... */ }`
          ),
        },
        links: [
    ...
    messages: {
      'attr-select':
        '.attr("{{string}}") is only for the initial html ' +
  ...
  create: detectJQueryCollections(
    (context, { foundAPositiveMatch, foundOnlyPositiveMatches }) => ({
      'CallExpression:exit': node => {
        ...
        if (!arg0 || !BANNED_ATTRS.includes(arg0.value)) {
          return
        }
