  ...
  meta: {
    ...
    messages: {
      'no-trailing-text-in-html-strings':
        'Trailing text after last tag in HTML strings is ignored',
      'no-trailing-text-in-html-strings-via-i18n':
        'Trailing text after last tag in HTML I18n strings is ignored',
    },
    type: 'problem',
  },
  create: context => {
    ...
    return detectDangerousI18n(options)(
      detectJQueryCollections(
        (context, { foundAPositiveMatch }, { couldBeDangerousHtmlString }) => {
          return {
            'CallExpression:exit': node => {
              if (!foundAPositiveMatch(node)) {
                return
              }
              ...
              node.arguments
                .map(arg =>
                  resolve((node, context, UNRESOLVED) => {
                    switch (node.type) {
                      case 'CallExpression':
                ...
                .forEach(value => {
                  ...
                  const messageId = value.includes(badI18nString)
                    ? 'no-trailing-text-in-html-strings-via-i18n'
                    : 'no-trailing-text-in-html-strings'
