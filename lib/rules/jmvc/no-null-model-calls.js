const {
  resolve,
  badValueWalker,
  detectIdentifierInMemberExpression,
} = require('../../util')
...
module.exports = {
  meta: {
    ...
    messages: {
      'no-null-model-calls':
        'Cannot encode {{value}} in model call input on path {{path}}',
      'no-null-model-defaults':
        'Cannot encode {{value}} in model defaults on path {{path}}',
    },
    type: 'problem',
  },
  create: detectIdentifierInMemberExpression(({ name }) =>
    ...
    'CallExpression:exit': node => {
      ...
      if (calleeType === 'Identifier' && calleeName === 'extend') {
        ...
        badArgumentTuples = badArgumentTuples.filter(
          ({ badValue: { path } }) => path.split('/')[1] === 'defaults'
        )
      }

      if (badArgumentTuples.length < 1) {
        return
      }

      const { node: badNode, badValue } = badArgumentTuples[0]
